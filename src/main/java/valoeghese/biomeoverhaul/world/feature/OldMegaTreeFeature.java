...
public abstract class OldMegaTreeFeature<T extends FeatureConfig> extends TBOTreeFeature<T> {
	protected final int baseHeight;
	protected final BlockState log;
	protected final BlockState leaves;
	protected final int maxExtraHeight;

	public OldMegaTreeFeature(Function<Dynamic<?>, ? extends T> function, boolean emitNeighborBlockUpdates, int baseHeight, int maxExtraHeight, BlockState log, BlockState leaves) {
	...
	protected void makeSquaredLeafLayer(ModifiableTestableWorld modifiableTestableWorld, BlockPos pos, int radius, BlockBox box, Set<BlockPos> set) {
		int i = radius * radius;
			...
			for(int k = -radius; k <= radius + 1; ++k) {
				...
				if (l + m < 7 && l * l + m * m <= i) {
					...
					if (isAirOrLeaves(modifiableTestableWorld, blockPos)) {
						this.setBlockState(set, modifiableTestableWorld, blockPos, this.leaves, box);
					}
				}
			}
		}
	...
	protected void makeRoundLeafLayer(ModifiableTestableWorld modifiableTestableWorld, BlockPos pos, int radius, BlockBox box, Set<BlockPos> set) {
		int i = radius * radius;
